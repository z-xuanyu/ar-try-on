...
const initThreeJS = () => {
  ...
  // renderer.setClearColor(0x00000000, 0); // 设置为透明背景
  // 将渲染器DOM元素添加到页面
  ...
  navigator.mediaDevices
    ...
    .then((stream) => {
      ...
      video.value?.addEventListener("canplaythrough", () => {
        ...
        // 设置相机的宽高比
        // 创建 OrthographicCamera 并根据视频宽高更新相机边界
        ...
        camera.position.z = 100; // 确保相机距离平面几何体的距离合适
...
const drawGlasses = (landmarks, angleData) => {
  ...
  if (!glassesMeshe.value) return;
  glassesMeshe.value.position.set(nose[0]?.x, nose[0]?.y, 0);
